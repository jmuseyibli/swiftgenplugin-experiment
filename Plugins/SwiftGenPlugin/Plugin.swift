...
extension SwiftGenPlugin: XcodeBuildToolPlugin {
    func createBuildCommands(
        ...
        guard validate(configurations: [configuration], targetName: target.displayName) else {
            return []
        }
...
private extension SwiftGenPlugin {
    ...
    func validate(configurations: [Path], targetName: String) -> Bool {
        let availableConfigurations = configurations.filter { configuration in
            FileManager.default.fileExists(atPath: configuration.string)
        }

        guard !availableConfigurations.isEmpty else {
            Diagnostics.error("""
